...
# Define the common loop
while true; do
    PMC_COUNTERS=
    UBOX0_COUNTER=
    CBOX_COUNTERS=
    UBOX1_COUNTER=
    # Process 4 elements from PMC at once
    pmc_items=("${PMC[@]:0:4}")
    if [ ${#pmc_items[@]} -gt 0 ]; then
    for i in "${!pmc_items[@]}"; do
        item="${pmc_items[i]}"
        if [ $i -lt 4 ]; then
            # Process the first 4 items from PMC
            PMC_COUNTERS="$PMC_COUNTERS,$item:${PMC_REGISTERS[i]}"
        fi
    done
    PMC=("${PMC[@]:4}") # Remove the processed items
    fi

    PMC_COUNTERS="${PMC_COUNTERS#,}"

    # Process 4 elements from CBOX at once
    cbox_items=("${CBOX[@]:0:6}")
    if [ ${#cbox_items[@]} -gt 0 ]; then
        ...
        CBOX=("${CBOX[@]:6}") # Remove the processed items
    fi
    CBOX_COUNTERS="${CBOX_COUNTERS#,}"

    # Process UBOX0 and UBOX1 events simultaneously
    if [ ${#UBOX0[@]} -gt 0 ]; then
        ubox0_item="${UBOX0[0]}"
        # Process 'ubox0_item' from UBOX0 and 'ubox1_item' from UBOX1
        UBOX0_COUNTER="$ubox0_item:UBOX0"
        UBOX0=("${UBOX0[@]:1}") # Remove the processed item from UBOX0
    fi
    ...
    if [ ${#UBOX1[@]} -gt 0 ]; then
        ...
        UBOX1=("${UBOX1[@]:1}") # Remove the processed item from UBOX1
    fi

    # Exit the loop when all arrays are empty
    if [ ${#PMC[@]} -eq 0 ] && [ ${#CBOX[@]} -eq 0 ] && [ ${#UBOX0[@]} -eq 0 ] && [ ${#UBOX1[@]} -eq 0 ]; then
        break
    fi
    # Loop through the command line arguments and append them to the command

    likwid_command="likwid-perfctr -t 10ms -C 0-3 -g "

    # Append FIXED_COUNTERS if it's not empty
    if [ -n "$FIXED_COUNTERS" ]; then
        likwid_command+="$FIXED_COUNTERS"
    fi

    # Append PMC_COUNTERS if it's not empty
    if [ -n "$PMC_COUNTERS" ]; then
        likwid_command+=",$PMC_COUNTERS"
    fi

    # Append CBOX_COUNTERS if it's not empty
    if [ -n "$CBOX_COUNTERS" ]; then
        likwid_command+=",$CBOX_COUNTERS"
    fi

    # Append UBOX0_COUNTER if it's not empty
    if [ -n "$UBOX0_COUNTER" ]; then
        likwid_command+=",$UBOX0_COUNTER"
    fi

    # Append UBOX1_COUNTER if it's not empty
